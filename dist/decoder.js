...
(function (State) {
    ...
    State[State["HEADER"] = 1] = "HEADER";
    State[State["PART_1"] = 2] = "PART_1";
    State[State["PART_2"] = 3] = "PART_2";
    State[State["PART_3"] = 4] = "PART_3";
    State[State["SYNOPSIS"] = 5] = "SYNOPSIS";
    State[State["ICE_REPORT"] = 6] = "ICE_REPORT";
    State[State["FORECAST"] = 7] = "FORECAST";
    State[State["END"] = 8] = "END";
})(State || (State = {}));
// Вспомогательные функции для КН-01
function decodeWMOGroup(group) {
...
function decodeTemperatureGroup(group) {
    const sn = group[1] === "1" ? "минус" : "+";
    const temp = parseInt(group.substring(2)) / 10;
    return `${sn}${temp} градусов`;
}
function decodeDewPointGroup(group) {
    const sn = group[1] === "1" ? "минус" : "+";
    const temp = parseInt(group.substring(2)) / 10;
    return `${sn}${temp} градусов`;
}
function decodeVisibilityGroup(group) {
...
function decodePressureGroup(group) {
    const pressure = parseInt(group.substring(1)) / 10 + 1000;
    return `${pressure} гПа`;
}
function decodePrecipitationGroup(group) {
...
function decodeMessage(input) {
    ...
    for (const line of lines) {
        ...
        if (upperLine.startsWith('ZCZC')) {
            if (((_a = context.message.sections) === null || _a === void 0 ? void 0 : _a.header.length) || ((_b = context.message.sections) === null || _b === void 0 ? void 0 : _b.part1.length) || ((_c = context.message.sections) === null || _c === void 0 ? void 0 : _c.part3.length)) {
            ...
            if (((_d = context.message.sections) === null || _d === void 0 ? void 0 : _d.header.length) || ((_e = context.message.sections) === null || _e === void 0 ? void 0 : _e.part1.length) || ((_f = context.message.sections) === null || _f === void 0 ? void 0 : _f.part3.length)) {
                messages.push(context.message);
            }
            context = {
                state: State.HEADER,
    ...
    if (((_g = context.message.sections) === null || _g === void 0 ? void 0 : _g.header.length) || ((_h = context.message.sections) === null || _h === void 0 ? void 0 : _h.part1.length) || ((_j = context.message.sections) === null || _j === void 0 ? void 0 : _j.part3.length)) {
        messages.push(context.message);
    }
    lastInput = input;
...
function processLine(line, context) {
    var _a, _b;
    const upperLine = line.toUpperCase();
    const appendLine = (text, indentLevel = 1) => {
        if (!context.message.sections)
            context.message.sections = { header: [], part1: [], part3: [] };
            ...
            if (context.message.type === 'SafetyNet') {
                if (upperLine.includes('GALE WARNING') || upperLine.includes('STORM WARNING') || upperLine.includes('CYCLONE WARNING')) {
                    ...
                    if (upperLine.includes('WINDS')) {
                        ...
                        const fromText = line.trim();
                        context.currentForecast.timeRange = { from: fromText, to: '' };
                        appendLine(i18n.__('time_range', { from: i18n.__(fromText.toLowerCase()), to: '' }));
                        context.message.sections.part1.push(context.currentForecast);
                        context.currentForecast = { stationCodes: [], timeRange: { from: fromText, to: '' }, wind: { direction: '', speed: '' } };
                    }
                }
        ...
        case State.PART_1:
            if (context.message.type === 'SafetyNet') {
                if (upperLine.includes('SYNOPSIS') || upperLine.includes('PART 2')) {
                    ...
                    const rangeText = line.trim();
                    context.currentForecast.timeRange = { from: rangeText, to: '' };
                    appendLine(i18n.__('time_range', { from: i18n.__(rangeText.toLowerCase()), to: '' }));
                    context.message.sections.part1.push(context.currentForecast);
                    context.currentForecast = { stationCodes: [], timeRange: { from: rangeText, to: '' }, wind: { direction: '', speed: '' } };
                }
            ...
            if (context.message.type === 'SafetyNet') {
                if (upperLine.includes('SYNOPSIS')) {
                    ...
                    context.message.sections.part2.synopsis.pressures.push({
                        ...
                        hpa: '',
                        position: line,
                        movement: upperLine.includes('MOVING') ? (_a = line.match(/MOVING\s+(.+)/i)) === null || _a === void 0 ? void 0 : _a[1] : undefined,
                    });
                    appendLine(line);
                }
            }
            ...
            if (context.message.type === 'SafetyNet') {
                if (upperLine.includes('FORECAST') || upperLine.includes('PART 3')) {
                    ...
                    context.message.sections.part2.synopsis.pressures.push({
                        ...
                        hpa: '',
                        position: line,
                        movement: upperLine.includes('MOVING') ? (_b = line.match(/MOVING\s+(.+)/i)) === null || _b === void 0 ? void 0 : _b[1] : undefined,
                    });
                    appendLine(line);
                }
            }
        ...
        case State.PART_3:
            if (context.message.type === 'SafetyNet') {
                if (/^\d{5}(,\s*\d{5})*/.test(line)) {
                    ...
                    const rangeText2 = line.trim();
                    context.currentForecast.timeRange = { from: rangeText2, to: '' };
                    appendLine(i18n.__('time_range', { from: i18n.__(rangeText2.toLowerCase()), to: '' }));
                    context.message.sections.part3.push(context.currentForecast);
                    context.currentForecast = { stationCodes: [], timeRange: { from: rangeText2, to: '' }, wind: { direction: '', speed: '' } };
                }
                else if (upperLine.includes('PETER THE GREAT GULF') || upperLine.includes('REGION')) {
                    if (!context.currentForecast) {
                        context.currentForecast = { stationCodes: [], timeRange: { from: '', to: '' }, wind: { direction: '', speed: '' } };
...
// Декодирование КН-01
function decodeKN01(lines, context) {
...
function formatMessage(messages) {
    ...
    messages.forEach((message, index) => {
        ...
        // Заголовок
        if ((_a = message.sections) === null || _a === void 0 ? void 0 : _a.header) {
        ...
        // Часть 1
        if ((_b = message.sections) === null || _b === void 0 ? void 0 : _b.part1) {
            appendLine(`=== ${i18n.__('part_1')} ===`);
            message.sections.part1.forEach(forecast => {
                if (forecast.region)
                    appendLine(i18n.__(forecast.region.toLowerCase()), 1);
                if (forecast.stationCodes.length)
                    appendLine(`${forecast.stationCodes.join(', ')}`, 1);
                ...
                if (forecast.temperature)
                    appendLine(`${i18n.__('temperature')}: ${forecast.temperature}`, 1);
                if (forecast.visibility)
                    appendLine(`${i18n.__('visibility')}: ${forecast.visibility}`, 1);
                if (forecast.seas)
                    appendLine(`${i18n.__('seas')}: ${forecast.seas}`, 1);
                if (forecast.iceAccretion)
                    appendLine(`${i18n.__('ice_accretion')}: ${forecast.iceAccretion}`, 1);
                if (forecast.pressure)
                    appendLine(`${i18n.__('pressure')}: ${forecast.pressure}`, 1);
                if (forecast.precipitation)
                    appendLine(`${i18n.__('precipitation')}: ${forecast.precipitation}`, 1);
            });
        }
        // Часть 2
        if ((_c = message.sections) === null || _c === void 0 ? void 0 : _c.part2) {
        ...
        // Часть 3
        if (((_d = message.sections) === null || _d === void 0 ? void 0 : _d.part3) && message.sections.part3.length > 0) {
